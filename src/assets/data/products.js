import productImg01 from "../images/double-sofa-01.png";
import productImg02 from "../images/double-sofa-02.png";
import productImg03 from "../images/double-sofa-03.png";

import productImg04 from "../images/single-sofa-01.jpg";
import productImg05 from "../images/single-sofa-02.jpg";
import productImg06 from "../images/single-sofa-03.jpg";
import productImg007 from "../images/single-sofa-04.png";

import productImg07 from "../images/arm-chair-01.jpg";
import productImg08 from "../images/arm-chair-02.jpg";
import productImg09 from "../images/arm-chair-03.jpg";
import productImg10 from "../images/arm-chair-01.jpg";

import productImg13 from "../images/phone-01.jpg";
import productImg14 from "../images/phone-02.jpg";
import productImg15 from "../images/phone-03.png";
import productImg16 from "../images/phone-04.jpg";
import productImg17 from "../images/phone-05.jpg";
import productImg18 from "../images/phone-06.jpg";

import productImg19 from "../images/watch-01.jpg";
import productImg20 from "../images/watch-02.jpg";
import productImg21 from "../images/watch-03.jpg";
import productImg22 from "../images/watch-04.jpg";

import productImg23 from "../images/wireless-01.png";

import productImg25 from "../images/wireless-03.png";

const products = [
  {
    id: "01",
    productName: "Stone and Beam Westview ",
    imgUrl: productImg01,
    category: "sofa",
    price: 193,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.7,
        text: "This product is very good",
      },
    ],
    avgRating: 4.5,
  },

  {
    id: "02",
    productName: "Rivet Bigelow Modern ",
    imgUrl: productImg02,
    category: "sofa",
    price: 253,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.8,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "03",
    productName: "Amazon Brand Modern Sofa",
    imgUrl: productImg03,
    category: "sofa",
    price: 173,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "26",
    productName: "Rivet Bigelow Modern ",
    imgUrl: productImg02,
    category: "sofa",
    price: 253,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.8,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "04",
    productName: "Fllufy Sheep Sofa",
    imgUrl: productImg04,
    category: "sofa",
    price: 163,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "05",
    productName: "Faux Velvet Sofa",
    imgUrl: productImg05,
    category: "sofa",
    price: 163,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "06",
    productName: "Fllufy Sheep Sofa",
    imgUrl: productImg06,
    category: "sofa",
    price: 163,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "07",
    productName: "Sakarias Armchair",
    imgUrl: productImg07,
    category: "chair",
    price: 99,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "27",
    productName: "Modern Arm Sofa",
    imgUrl: productImg007,
    category: "sofa",
    price: 173,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "08",
    productName: "Baltsar Chair",
    imgUrl: productImg08,
    category: "chair",
    price: 89,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "09",
    productName: "Helmar Chair",
    imgUrl: productImg09,
    category: "chair",
    price: 112,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "10",
    productName: "Apple iPhone 12 Pro",
    imgUrl: productImg13,
    category: "mobile",
    price: 799,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },
  {
    id: "25",
    productName: "Sakarias Armchair",
    imgUrl: productImg10,
    category: "chair",
    price: 99,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.6,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "11",
    productName: "Apple iPhone 12 Max",
    imgUrl: productImg14,
    category: "mobile",
    price: 799,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "12",
    productName: "Realme 8",
    imgUrl: productImg15,
    category: "mobile",
    price: 599,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "13",
    productName: "One Plus Nord",
    imgUrl: productImg16,
    category: "mobile",
    price: 799,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "14",
    productName: "Apple iPhone 13 Pro",
    imgUrl: productImg17,
    category: "mobile",
    price: 899,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "15",
    productName: "Samsung Galaxy S22",
    imgUrl: productImg18,
    category: "mobile",
    price: 699,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "16",
    productName: "Rolex Watch",
    imgUrl: productImg19,
    category: "watch",
    price: 299,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "17",
    productName: "Timex Easy Reader Watch",
    imgUrl: productImg20,
    category: "watch",
    price: 299,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "18",
    productName: "Rolex Watch",
    imgUrl: productImg21,
    category: "watch",
    price: 299,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "19",
    productName: "Apple Watch",
    imgUrl: productImg22,
    category: "watch",
    price: 399,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "20",
    productName: "Beat Studio Wireless",
    imgUrl: productImg23,
    category: "wireless",
    price: 199,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "22",
    productName: "Beat EP Headphones",
    imgUrl: productImg25,
    category: "wireless",
    price: 199,
    shortDesc:
      "The company itself is a very successful company. They have the right to achieve the pleasures of that time of work!",
    description:
      "The company itself is a very successful company. This is the choice of our accusers, what pleasure they provide! Let the duties of the soul be followed by the matter at the time of pleasure, and I will open here and the like, so that the distinction of repudiating may be followed, because I hate those whom, which, justly, needs, I come to hate, the training which one owes to pleasures is laborious! You must be blinded by flattery, but it hinders some!",
    reviews: [
      {
        rating: 4.8,
        text: "This product is very good"
      },
      {
        rating: 4.9,
        text: "This product is very good"
      },
    ],
    avgRating: 4.8,
  },
];

export default products;
